"""Prepare input batches, based on
...
NUM_CHANNELS = 3
TRAIN_SHARDS_NUM = 1024
VAL_SHARDS_NUM = 128

def get_filenames(is_train_dataset, data_dir):
    ...
    if (is_train_dataset):
        data_filemames = [os.path.join(data_dir, 'train-%05d-of-%05d' % (i, TRAIN_SHARDS_NUM)) for i in range(TRAIN_SHARDS_NUM)]
    else:
        data_filemames = [os.path.join(data_dir, 'validation-%05d-of-%05d' % (i, VAL_SHARDS_NUM)) for i in range(VAL_SHARDS_NUM)]
...
def parse_example_proto(example_serialized, adv_eval_data=False):
    """Parse an Example proto that corresponds to an image.
       ...
       image/channels: 3
       image/class/label: 1 - 1000 # label value 0 was left empty for the background class when building the dataset
    ...
    feature_map = {
        ...
        'image/class/synset': tf.io.FixedLenFeature([], dtype=tf.string, default_value='')
    }
    # Extract sparse features in Example proto
    sparse_float32 = tf.io.VarLenFeature(dtype=tf.float32)
    feature_map.update(
        {k: sparse_float32 for k in ['image/object/bbox/xmin', 'image/object/bbox/ymin',
    ...
    # Make the variable number of bounding boxes into
    # the shape [1, num_boxes, coords]
    bbox = tf.concat([ymin, xmin, ymax, xmax], 0)
    bbox = tf.expand_dims(bbox, 0)
    bbox = tf.transpose(a=bbox, perm=[0, 2, 1])

    if (not adv_eval_data):
        to_batch = (encoded_img, label, bbox)
    else:
        img_filename = features['image/filename']
        img_synset = features['image/class/synset']
        img_label_text = features['image/class/text']
        to_batch = (encoded_img, label, bbox, img_filename, img_synset, img_label_text)
...
def distort_color(image, color_ordering):
    ...
    Args:
        ...
        color_ordering: int; denotes the kind of color distortion.
    Returns:
        image: 3-D float Tensor; it contains an image. It is of
            size [H, W, C], where H is the image height, W is
            the image width, and C is the number of channels.
    ...
    if color_ordering == 0:
        ...
        raise ValueError('color_ordering must be in [0, 4]')
...
def distort_image(image_buffer, output_height, output_width, num_channels, bbox):
    ...
    # Perform random color distortions
    distorted_image = apply_with_random_selector(distorted_image,
                                                lambda x, color_ordering: distort_color(x, color_ordering),
                                                cases=np.arange(5))

    return distorted_image

def preprocess_image(image_buffer, bbox, output_height, output_width,
                     num_channels, is_train_dataset, is_training):
    """Preprocess an image.
    Args:
...
def parse_record(raw_record, is_train_dataset, is_training,
    ...
    Returns:
        batch: tuple; it contains the following entries:
            ...
            label: int; numeric image label.
            img_filename (optional): string; the filename of an image.
            img_synset (optional): string; the synset of an image.
            img_label_text (optional): string; the human-readable label of an image.
    """
    
    # Parse Example protocol buffer
    if (not adv_eval_data):
        image_buffer, label, bbox = parse_example_proto(raw_record, adv_eval_data)
    else:
        (image_buffer, label, bbox,
        img_filename, img_synset, img_label_text) = parse_example_proto(raw_record, adv_eval_data)
    ...
    # Subtract 1 so that labels are in [0, 1000) range
    ...
    if (not adv_eval_data):
        ...
        batch  = (image, label, img_filename, img_synset, img_label_text)
...
def process_record_dataset(dataset,
    ...
    # Parse raw records
    dataset = dataset.map(lambda value: parse_record_fn(value, is_train_dataset, is_training,
                                                        img_size_y, img_size_x, dtype, adv_eval_data),
...
def input_fn(is_train_dataset,
    ...
    if (filenames is None):
        filenames = get_filenames(is_train_dataset, data_dir)
