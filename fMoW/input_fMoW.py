...
def get_filenames(dataset_type, data_dir):
    ...
    if (dataset_type == 'train'):
        ...
        data_filemames = [os.path.join(data_dir, 'test-%05d-of-%05d' % (i, TEST_SHARDS_NUM)) for i in range(TEST_SHARDS_NUM)]
...
def parse_example_proto(example_serialized, adv_eval_data=False):
    ...
    if (not adv_eval_data):
        ...
        to_batch = (encoded_img, label, bbox, img_filename, img_label_text)
...
def input_fn(dataset_type,
    ...
    if (is_training):
        if (dataset_type == 'train'):
            ...
            dataset = dataset.shuffle(buffer_size=TEST_SHARDS_NUM)
