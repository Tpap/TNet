...
class distributedTrainer():
    def __init__(self):
        ...
        self.model = tf_models.TNet(FLAGS.descr_tag, FLAGS.ls_dim, FLAGS.num_patches_y, FLAGS.num_patches_x,
                                    ...
                                    FLAGS.dropout_rate, FLAGS.loc_per_grid, FLAGS.width_coefficient, FLAGS.depth_coefficient,
                                    FLAGS.block_drop_rate, FLAGS.depth_divisor, FLAGS.activation, FLAGS.batch_norm, FLAGS.pos_dim_divisor)
    ...
    def update_adv_eval_data(self, step_batch, ret_obj):
        ...
        preds = [tf.cast(tf.math.argmax(l, axis=1), tf.int32) for l in logits]
        self.preds_lst.extend(preds)
        
        max_probs = [tf.cast(tf.math.reduce_max(tf.nn.softmax(l), axis=1), tf.float32) for l in logits]
        self.max_probs_lst.extend(max_probs)
    ...
    def save_adv_eval_data(self, dataset_type):
        ...
        max_probs = tf.concat(self.max_probs_lst, axis=0).numpy()
        category_names_np = np.array(category_names)
        human_preds = category_names_np[preds_cls]
        ...
        cols = ['fnames', 'labels', 'human_labels', 'predicted_class', 'human_preds',
                'locations_num', 'locations', 'locations_probs', 'top1_mask', 'top5_mask', 'max_prob']
...
def main(argv=None):
    ...
    with strategy.scope():
        ...
        if (FLAGS.to_evaluate_test):
            ...
            trainer.evaluate(dataset_type='test', is_training=False, num_epochs=FLAGS.eval_epochs_num, adv_eval_data=FLAGS.adv_eval_data)
